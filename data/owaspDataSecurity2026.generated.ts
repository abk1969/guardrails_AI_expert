...
export const owaspDataSecurity2026KeyItems: ReferenceItem[] = [
  {
    ...
    "priority": "medium",
    "description": "AI-DSPM (Data Security Posture Management) for GenAI is the continuous practice of discovering, classifying, governing, and monitoring data across GenAI pipelines and runtimes, so you can see where sensitive data exists, understand how it…"
  },
  {
    "id": "dsgai01",
    "code": "DSGAI01",
    ...
    "detailedSections": {
      ...
      "mitigationTiers": {
        "tier1": [
          ...
          "Output controls: Implement PII/secret detectors on generations. Monitor for high-risk patterns with blocklists/regex. Beware of Cross-Lingual Leakage Risk, which renders regex controls useless if an attacker asks a reply in another language, in binary, or sends an encoded message. Scope: Buy and Build.",
      ...
      "knownCVEs": [
        ...
        "The Jailbreak That Talked Back",
        ...
        "Neuraltrust",
        "\"Why Your AI Model Might Be Leaking Sensitive Data\" https://neuraltrust.ai/blog/ai-model-data-leakage-prevention"
      ]
    },
    "priority": "critical",
    "description": "An attacker (or a curious user) interacts with the model or a RAG system that was trained on to retrieve sensitive corporate data. Through carefully crafted instructions, enumeration, or high-recall prompts, the system returns verbatim or…"
  },
  {
    "id": "dsgai02",
    "code": "DSGAI02",
    ...
    "detailedSections": {
      ...
      "knownCVEs": [
        ...
        "CVE-2025-54795: An error in command parsing makes it possible to bypass the Claude Code confirmation prompt to trigger execution of an untrusted command. More details: https://www.wiz.io/vulnerability-database/cve/cve-2025-54795 For broader identity-based privilege abuse patterns -- including human operator privilege escalation, role confusion attacks, and cross-tenant identity bleed -- see cross-reference: ASI03 -- Identity & Privilege Abuse (OWASP GenAI Security Project - Agentic Top 10)"
      ]
    },
    "priority": "critical",
    "description": "AI agent pipelines generate a rapidly expanding surface of Non-Human Identities (NHIs) -- service accounts, API keys, OAuth tokens, and tool credentials -- that accumulate across orchestration layers with little lifecycle governance. The…"
  },
  {
    "id": "dsgai03",
    "code": "DSGAI03",
    ...
    "detailedSections": {
      "overview": "Business units experiment with public AI SaaS tools, browser plugins, and \"productivity\" agents without security review. Employees paste customer records, internal designs, or source code into these tools. Vendors may retain and train on this data, store it in third-country regions, or expose it via misconfigurations. No formal lineage, DSR handling, or contractual guarantees exist. Shadow AI encompasses a significantly broader threat surface than the intuitive image of an employee pasting text into ChatGPT. It spans any AI-enabled capability adopted outside formal security and procurement governance, and manifests across several distinct categories. The most visible form is consumer and prosumer GenAI SaaS -- public-facing tools such as ChatGPT, Copilot, Gemini, or browser- based writing and coding agents -- where employees submit sensitive prompts, documents, customer records, or source code without contractual data handling guarantees, enforceable retention limits, or visibility into whether inputs are used for model training. A second, less-examined category is third-party productivity tooling with embedded AI -- CRM plugins, email assistants, document summarizers, or sales enablement tools that surface AI capabilities as a secondary feature, often without the adopting team recognizing that a data processing relationship with an external model provider has been created at all. A third and increasingly prevalent category is startup and niche ML tools, where a vendor has built a domain- specific capability -- product ranking, document classification, fraud scoring -- on top of open-source or hosted models (such as Hugging Face pipelines) with infrastructure deployed opportunistically on public cloud regions chosen for cost rather than compliance. In these cases, the data residency may fall outside jurisdictions covered by the organization's data protection obligations, meaning deletion requests, subject access rights, and legal data recovery mechanisms operate under foreign law -- or may not be enforceable at all. A fourth category is internally built but ungoverned AI tooling, where individual teams or developers stand up inference endpoints, fine-tuning pipelines, or retrieval systems within corporate cloud tenants without security review, data classification, or alignment with enterprise AI policy. A fifth and continuously rising category are AI features which are added to formally procured IT applications with originally no AI capabilities, where AI Governance functions are not informed about the newly added AI capabilities. Across all of these categories, the common thread is the absence of the controls that formal AI procurement would otherwise mandate -- data lineage, contractual data handling guarantees, DSR compliance, output monitoring, and audit rights -- and it is precisely this gap that makes Shadow AI a data security risk distinct from, but deeply interconnected with, the controls addressed in some other entries in this document.",
      "attackVectors": [
        "Attackers in Shadow AI scenarios are often indirect beneficiaries rather than active intruders. The primary capability required is the ability to operate or compromise an external AI service that employees voluntarily use. This includes SaaS providers that retain prompts for training, third-party plugins that forward full transcripts to backend services, or vendors that change data handling practices post-adoption. More sophisticated adversaries may target Shadow AI SaaS providers directly, exploiting weak access controls, cloud misconfigurations, or credential leaks to harvest sensitive corporate prompts and documents aggregated from multiple customers. Because the organization has no visibility into these environments, detection and response capabilities are effectively externalized. In internal \"shadow build\" cases, attackers require only standard corporate access to deploy unreviewed inference endpoints or retrieval systems. Once deployed without governance, these systems become soft targets for lateral movement, credential harvesting, or data scraping."
      ],
      "examples": [
      ...
      "references": [
        "Palo Alto Networks",
        "\"What Is Shadow AI? How It Happens and What to Do About It\" https://www.paloaltonetworks.ca/cyberpedia/what-is-shadow-ai Palo Alto Networks",
        "\"Four Ways Your Organization Can Lower the Security Risk From Shadow AI\" https://www.ibm.com/think/insights/security-risk-shadow-AI ibm.com",
        "\"How leaders can govern Shadow AI\" (definition and risks of ungoverned AI tool use) https://sosafe-awareness.com/blog/shadow-ai-risks-culture-safe-ai-use/ SoSafe",
        "\"Why you need to address Shadow AI--and how to get started\" https://gdt.com/blog/why-you-need-to-address-shadow-ai-and-how-to-get-started/ GDT",
        "Shadow AI: \"The Next Generation of Shadow IT\" Shadow AI: The Next Generation of Shadow IT | BDO"
      ]
    },
    "priority": "high",
    "description": "Business units experiment with public AI SaaS tools, browser plugins, and \"productivity\" agents without security review. Employees paste customer records, internal designs, or source code into these tools. Vendors may retain and train on…"
  },
  {
    "id": "dsgai04",
    "code": "DSGAI04",
    ...
    "detailedSections": {
      ...
      "knownCVEs": [
        ...
        "No dedicated CVE for artifact tampering or hyperparameter manipulation; these are pipeline integrity failures rather than discrete software vulnerabilities. This entry extends LLM03:2025 (LLM Supply Chain) and ASI04 with a data-integrity lens, focusing on the training corpus and artifact pipeline attack surface. For identity and credential controls on the supply chain access layer, see DSGAI02 (Agent Identity & Credential Exposure). For disinformation introduced via trusted retrieval sources at runtime, see DSGAI26 (Disinformation & Integrity Attacks via Data Poisoning)."
      ]
    },
    "priority": "critical",
    "description": "The full attack lifecycle moves through three stages that are operationally distinct but technically continuous. Understanding them separately is useful for detection; treating them as separate risks is not. Stage 1 -- Supply chain…"
  },
  {
    "id": "dsgai05",
    "code": "DSGAI05",
    ...
    "detailedSections": {
      ...
      "mitigationTiers": {
        "tier1": [
          ...
          "Non-root services and read-only mounts: AI infrastructure components run as non-root with minimal filesystem write scope. Scope: Build."
        ],
        "tier2": [
    ...
    "priority": "high",
    "description": "AI pipelines ingest data from a wide range of sources -- uploaded files, API feeds, snapshot imports, labeling queues, and third-party data connectors -- and the validation applied at each ingestion boundary is frequently insufficient to…"
  },
  {
    "id": "dsgai06",
    "code": "DSGAI06",
    ...
    "detailedSections": {
      ...
      "references": [
        "\"ChatGPT Has a Plug-In Problem\" https://www.wired.com/story/chatgpt-plugins-security-privacy-risk/ WIRED",
        "Salt Security",
        "\"Security Flaws within ChatGPT Ecosystem Allowed Access to Accounts on Third- Party Websites and Sensitive Data\" https://salt.security/blog/security-flaws-within-chatgpt- extensions-allowed-access-to-accounts-on-third-party-websites-and-sensitive-data salt.security",
        "\"12 Key ChatGPT Security Risks with Examples\" (plugins & third-party integrations) https://www.reco.ai/learn/chatgpt-security-risk",
        "TheHackerNews - \"First Malicious MCP Server Found Stealing Emails in Rogue Postmark-MCP Package https://thehackernews.com/2025/09/first-malicious-mcp-server- found.html?utm_source=chatgpt.com"
      ]
    },
    "priority": "critical",
    "description": "Every tool invocation, plugin call, or agent handoff is a potential exfiltration boundary. When an AI assistant calls a plugin, forwards a task to a sub-agent, or receives instructions from an orchestrator, it typically shares all or part…"
  },
  {
    "id": "dsgai07",
    "code": "DSGAI07",
    ...
    "detailedSections": {
      ...
      "mitigations": [
        ...
        "Erasure scope must include derived artifacts explicitly: Deletion workflows must enumerate and act on all derived artifacts, not only source records. This requires maintaining a live inventory of artifact-to-source relationships so that a deletion event can propagate to embeddings, index entries, backups, and any training artifacts that ingested the record. Simulated deletion tests -- delete a test record and verify no trace in DB, logs, embeddings, backups, and model outputs -- should be a standard validation step in pipeline design.",
    ...
    "priority": "high",
    "description": "In conventional data systems, a governance failure -- missing classification label, expired retention policy, absent lineage record -- is contained to the layer where it occurs. In AI pipelines, the same failure propagates forward and…"
  },
  {
    "id": "dsgai08",
    "code": "DSGAI08",
    ...
    "detailedSections": {
      ...
      "mitigations": [
        ...
        "EU AI Act readiness planning: For organizations deploying high-risk AI systems, Article 10 training data governance requirements enter force in August 2026. The data governance controls required by this framework -- lineage, classification, quality documentation, and bias evaluation for training data -- directly satisfy Article 10 obligations. Organizations should treat this deadline as a driver for accelerating DSGAI07 implementation. Regulatory mapping Full framework mapping is maintained in the companion implementation guide. Key obligations directly relevant to this entry:",
        ...
        "EU AI Act Article 10 (training data governance for high-risk systems, August 2026); Articles 13",
        "14 (transparency and human oversight)",
    ...
    "priority": "medium",
    "description": "Regulatory exposure in AI systems rarely originates as a discrete event -- it surfaces when technical risks documented elsewhere in this framework go unmitigated and intersect with enforceable legal obligations. Three failure patterns are…"
  },
  {
    "id": "dsgai09",
    "code": "DSGAI09",
    ...
    "detailedSections": {
      ...
      "attackVectors": [
        "Adversaries exploiting multimodal capture risks do not rely solely on text-based extraction techniques. Instead, they take advantage of the fact that images, audio, video, and scanned documents often bypass mature text-centric controls and are processed through parallel pipelines with weaker governance. At a basic level, an attacker can obtain access to object storage, logging systems, or embedding stores where raw media and their derivatives are retained. Because OCR outputs, transcripts, thumbnails, and extracted metadata are frequently stored separately from the original file -- and sometimes with lower classification -- compromising any one of these repositories can yield sensitive information such as credentials visible in screenshots, identity numbers from scanned documents, or confidential discussions captured in voice notes. More capable adversaries exploit cross-channel asymmetries. They understand that while images may be access-restricted, the extracted text may be indexed for search, logged for debugging, or reused for analytics. By targeting the derivative layer rather than the original media asset, attackers can recover high- value data without needing access to the raw file. Similarly, multimodal embeddings may encode structured signals (faces, document layouts, voice characteristics) that can be probed or reconstructed if storage protections are weak. Attackers may also combine signals across modalities to increase inference power. A partial name visible on a whiteboard, a badge number in a photo, and a voice recording with identifiable characteristics can be correlated to re-identify individuals or reconstruct sensitive operational context. This cross-modal aggregation capability significantly increases risk compared to single-channel leakage. More advanced actors leverage model inversion or embedding probing techniques to recover latent details from multimodal representations, particularly when embeddings are exposed via search APIs or shared across services. Even without direct storage compromise, systematic querying can sometimes surface sensitive fragments embedded during prior processing. Across all cases, the attacker's advantage stems from exploiting the expanded ingestion surface and derivative sprawl introduced by multimodal AI. When images, audio, and video are treated as \"inputs for understanding\" rather than regulated data assets, adversaries can target overlooked storage paths, derivative artifacts, and cross-channel correlations to extract information that traditional text-focused controls would otherwise catch."
      ],
      "examples": [
      ...
      "references": [
        ...
        "\"Multimodal artificial intelligence\" https://www.edps.europa.eu/data-protection/technology-monitoring/techsonar/multimodal- artificial-intelligence European Data Protection Supervisor",
        "TechMonitor",
        "\"Multimodal AI models pose increased risks for abuse and harmful content\" https://www.techmonitor.ai/ai-and-automation/multimodal-ai-models-increased-risks-abuse- harmful-content/ Tech Monitor",
        "Pillar Security",
        "\"Securing Multimodal AI\" https://www.pillar.security/blog/securing-multimodal-ai"
      ]
    },
    "priority": "high",
    "description": "Users upload screenshots of dashboards, photos of whiteboards, scans of passports, PDFs, and voice notes so multimodal assistants can \"understand context.\" These files are processed and retained in object stores, embeddings, and logs. The…"
  },
  {
    "id": "dsgai10",
    "code": "DSGAI10",
    ...
    "detailedSections": {
      ...
      "mitigations": [
        ...
        "Machine unlearning planning: Where fine-tuned models are trained on data containing rare records, maintain versioned data-to-model links so that the contribution of specific cohorts can be traced. This enables targeted retraining if re-identification risk is later confirmed. See DSGAI01 for the technical constraints on machine unlearning and their compliance implications."
      ],
      "mitigationTiers": {
        "tier1": [
    ...
    "priority": "medium",
    "description": "The core failure is an assumption that data transformation -- de-identification, tokenization, normalization, or synthetic generation -- produces a privacy guarantee it does not actually provide. This manifests across the data pipeline in…"
  },
  {
    "id": "dsgai11",
    "code": "DSGAI11",
    ...
    "detailedSections": {
      ...
      "attackVectors": [
        "Adversaries exploiting cross-context and multi-user conversation bleed do not need privileged access to backend systems. Their leverage often comes from understanding how session state, caching, and retrieval scoping are implemented and where isolation assumptions break down. At a basic level, an attacker can perform context probing. By issuing structured prompts designed to elicit residual memory (\"What was the previous question?\" / \"Summarize the last uploaded document\"), they test whether the assistant retains state beyond the authenticated session boundary. If session identifiers are predictable, reused, or weakly bound to identity, attackers may also attempt session fixation or enumeration to inherit another user's active context. More technically capable actors exploit weaknesses in retrieval scoping and shared indexes. They craft queries likely to collide semantically with content belonging to other tenants -- especially in systems using shared vector stores with soft filters. If filtering is applied after retrieval rather than before ranking, attackers may trigger partial leakage, metadata exposure, or paraphrased fragments from another tenant's corpus. In shared inference environments, attackers may target serving-layer side channels. Where KV caches or response caches are reused across tenants for performance optimization, carefully timed or structured requests can expose fragments of prior prompts or completions. Such attacks rely on understanding serving internals rather than application-layer flaws. Advanced adversaries may combine these techniques with authorization boundary testing. By manipulating user roles, switching workspaces, or exploiting race conditions during permission updates, they probe for transient states where isolation checks fail. Because these issues often surface only under specific concurrency or caching conditions, attackers may automate repeated edge-case testing to trigger rare bleed events. Across all variants, the attacker's capability centers on systematically probing isolation guarantees, at the session layer, retrieval layer, and serving layer, to surface residual state or improperly scoped data that crosses tenant or user boundaries."
      ],
      "examples": [
      ...
      "references": [
        "\"Cross Session Leak: when your AI assistant becomes a data breach\" https://www.giskard.ai/knowledge/cross-session-leak-when-your-ai-assistant-becomes-a-data- breach Giskard+1",
        "NDSS 2025",
        "\"I Know What You Asked: Prompt Leakage via KV-Cache Sharing in Multi-Tenant LLM Serving\" (NDSS page) https://www.ndss-symposium.org/ndss-paper/i-know-what-you-asked- prompt-leakage-via-kv-cache-sharing-in-multi-tenant-llm-serving/ NDSS Symposium",
        "(PDF) https://www.ndss-symposium.org/wp-content/uploads/2025-1772-paper.pdf NDSS Symposium",
        "NeurIPS 2025 - \"Memory Injection Attacks on LLM Agents via Query-Only Interaction\" https://neurips.cc/virtual/2025/loc/san-diego/poster/118152",
        "(PDF)- \"INJECMEM: Memory Injection Attack on LLM Agent Memory Systems\" https://openreview.net/pdf?id=QVX6hcJ2um"
      ]
    },
    "priority": "critical",
    "description": "LLM systems reuse conversation state or shared memory across sessions, tenants, or users to create \"persistent assistants.\" Weak isolation between workspaces, mishandled conversation IDs, or bugs in session management cause content from…"
  },
  {
    "id": "dsgai12",
    "code": "DSGAI12",
    ...
    "detailedSections": {
      ...
      "attackVectors": [
        "Unsafe naturallanguage data gateways arise when large language models are granted the ability to translate freeform user input directly into structured database queries such as SQL or graph queries and execute them against live systems. These LLMtoSQL or LLMtoGraph interfaces are often deployed to enable \"chat with your database\" experiences, analytics assistants, or agentic workflows. While they improve accessibility, they collapse the traditional security boundary between user input and database logic, creating a new class of injection and logic abuse attacks that do not rely on classic SQL syntax manipulation but instead exploit the LLM's instruction following behavior. A primary attack capability is prompttoquery injection, where the attacker uses natural language to coerce the model into generating harmful queries. Unlike traditional SQL injection, the attack does not require malformed input or special characters. The entire user prompt becomes the attack surface, allowing grammatically valid instructions such as requests to \"also delete,\" \"clean up,\" or \"reset\" data to be interpreted by the model as legitimate intent. These gateways also enable privilege amplification through model authority. LLMgenerated queries often execute under a single, highly privileged service account rather than the end user's identity. As a result, even a low privilege user can indirectly perform high impact operations by convincing the model to generate queries that exceed their authorized access. LLMtoSQL and LLMtoGraph systems are also vulnerable to modellevel backdoor and poisoning attacks. Research shows that Text to SQL models can be fine tuned or influenced with poisoned data that introduces hidden triggers. When these triggers appear in otherwise benign prompts, the model generates malicious or injection style queries while maintaining normal behavior for standard inputs."
      ],
      "examples": [
      ...
      "knownCVEs": [
        "CVE-2024-8309 (LangChain): A prompt injection vulnerability in GraphCypherQAChain. An attacker can manipulate the LLM to generate malicious Cypher queries, leading to unauthorized data access or modification in Neo4j databases.",
      ...
      "references": [
        "\"How to safely use LLMs for Text-to-SQL with Stored Procedures\" https://erincon01.medium.com/how-to-safely-use-llms-for-text-to-sql-with-stored-procedures- ba7540067f5f Medium",
        "DigitalAPI",
        "\"How to expose APIs to LLMs without breaking security\" https://www.digitalapi.ai/blogs/expose-apis-to-llms digitalapi.ai"
      ]
    },
    "priority": "high",
    "description": "Organizations expose data warehouses, graphs, and analytics engines via \"Ask your data\" copilots. The LLM is allowed to generate raw SQL/GraphQL/REST queries over wide schemas. Prompt injection, hallucinated queries, or mis-specified…"
  },
  {
    "id": "dsgai13",
    "code": "DSGAI13",
    ...
    "priority": "critical",
    "description": "Unencrypted embeddings and permissive vector APIs allow similarity queries to surface sensitive content. Misconfigured and permissive vector APIs, unencrypted data at rest, or long-lived credentials allow unauthorized users to access and…"
  },
  {
    "id": "dsgai14",
    "code": "DSGAI14",
    ...
    "detailedSections": {
      "overview": "Teams instrument every request/response with rich logs, traces, and session captures to debug agent workflows and RAG quality. Logging middleware dumps full prompts, tool outputs, vector results, and credentials into centralized observability stacks. Highlight for third-party risk, if using external log analytic services, ensure they are secured to the same standard. Even limited data in logs can be sensitive. Disclosure reference from OpenAI's in November 2025, regarding Mixpanel https://openai.com/index/mixpanel-incident/ A compromise of the logging platform, mis-scoped search permissions, or an overly curious insider allows bulk access to conversation histories, secrets, and training samples that were never meant to be persisted.",
      "attackVectors": [
        "Adversaries targeting observability infrastructure recognize that logging and tracing platforms represent a uniquely high-value, often under-secured aggregation point: a single compromise yields not one user's data but the accumulated prompts, responses, credentials, and tool outputs of an entire system across weeks or months of operation. The attacker does not need to breach the primary data store -- they need only reach the observability stack, which frequently has weaker access controls, broader internal network exposure, and less rigorous data classification than the systems it monitors. At the opportunistic end, a phished or compromised analyst account with bulk log export permissions is sufficient to exfiltrate conversation histories, embedded secrets, and system prompt content at scale -- as the OpenAI Mixpanel incident illustrated. More targeted adversaries actively probe observability infrastructure as a preferred lateral movement destination precisely because logs aggregate sensitive data from multiple upstream systems in a single queryable interface. Insiders present a distinct and elevated risk in this context: a SOC analyst, ML engineer, or DevOps operator with legitimate access to the observability stack may have visibility into prompt and response content that far exceeds what their role requires, and bulk export of that data is indistinguishable from normal investigative activity without behavioral baselining. In agentic systems, the attack surface expands further -- agent traces capture not only user prompts but tool call parameters, retrieved document content, intermediate reasoning steps, and inter-agent communication payloads, meaning a single observability breach can expose the full operational context of an autonomous workflow including credentials passed between agents that were never intended to be logged."
      ],
      "examples": [
      ...
      "references": [
        "LLM Observability overview https://docs.datadoghq.com/llm_observability/docs.datadoghq.com",
        "Prompt tracking for LLM applications https://docs.datadoghq.com/llm_observability/monitoring/prompt_tracking/docs.datadoghq.com",
        "Product page: LLM Observability (tracing prompts, responses, intermediate steps) https://www.datadoghq.com/product/llm-observability/ Datadog"
      ]
    },
    "priority": "medium",
    "description": "Teams instrument every request/response with rich logs, traces, and session captures to debug agent workflows and RAG quality. Logging middleware dumps full prompts, tool outputs, vector results, and credentials into centralized…"
  },
  {
    "id": "dsgai15",
    "code": "DSGAI15",
    ...
    "detailedSections": {
      "overview": "To \"improve answers,\" teams pack prompts with full user profiles, tickets, transaction histories, and document blobs. LLM gateways automatically append rich context (e.g., \"customer_360\") to every request. These payloads are sent to external API providers, cached by edge services, and logged for debugging. A breach at the provider, misconfigured cache, or insider with log access now has a 360� view of sensitive user data that never needed to leave the core systems.",
      "attackVectors": [
        "Adversaries exploiting over-broad context windows do not need to defeat encryption or bypass model safeguards. Their advantage comes from gaining access to systems where excessive prompt payloads are routinely transmitted, stored, or observed. At a basic level, an attacker who compromises an upstream LLM provider account, edge cache, observability stack, or logging pipeline gains visibility into entire prompt bodies. When prompts systematically include full user profiles, transaction histories, internal notes, or document blobs, a single breach yields disproportionately rich datasets. Because the over-sharing is structural rather than accidental, every request becomes a high-density intelligence artifact. More capable adversaries target internal access paths. An insider with log access, a contractor with debugging privileges, or an attacker who gains foothold in monitoring infrastructure can harvest sensitive information from prompt telemetry without touching core databases. Attackers may also exploit framework defaults and configuration drift. If auto-context features expand silently over time such as appending additional fields, attachments, or historical threads, adversaries monitoring outbound traffic can detect and capitalize on this \"field creep,\" knowing that sensitive elements are likely included regardless of task necessity. In multi-provider architectures, adversaries can aggregate exposure across vendors. Even if no single prompt contains a full record, repeated over-scoped requests allow reconstruction of comprehensive user profiles through correlation of logs, cached payloads, or retained API traces. Ultimately, the attacker's capability is amplified by systemic prompt inflation. When excessive context is normalized as a design pattern, any compromise in the provider, logging, or observability chain yields access not just to answers -- but to complete underlying records that never needed to leave the originating system."
      ],
      "examples": [
      ...
      "references": [
        "ROW Privacy Policy (see sections on data sharing with third parties) https://openai.com/policies/row-privacy-policy OpenAI",
        "Private Internet Access",
        "\"ChatGPT and Privacy: Everything You Need to Know in 2025\" https://www.privateinternetaccess.com/blog/chatgpt-privacy/ Private Internet Access"
      ]
    },
    "priority": "high",
    "description": "To \"improve answers,\" teams pack prompts with full user profiles, tickets, transaction histories, and document blobs. LLM gateways automatically append rich context (e.g., \"customer_360\") to every request. These payloads are sent to…"
  },
  {
    "id": "dsgai16",
    "code": "DSGAI16",
    ...
    "detailedSections": {
      ...
      "knownCVEs": [
        "Research shows AI-enabled browsers and extensions can be hijacked via hidden prompt instructions or malicious updates, enabling data exfiltration and account takeover. LayerX+3TechRadar+3Dark Reading+3"
      ],
      "references": [
        "TechRadar",
        "\"AI browsers can be hacked with a simple hashtag, experts warn\" (HashJack) https://www.techradar.com/pro/thats-not-very-trendy-of-them-ai-browsers-can-be-hacked- with-a-simple-hashtag-experts-warn TechRadar",
        "Dark Reading",
        "\"AI Browser Extensions: The New Security Battleground\" https://www.darkreading.com/cyber-risk/ai-browser-extensions-security-battleground Dark Reading Seraphic",
        "\"AI Browser Extensions: Pros/Cons & 8 Extensions to Know\" (data privacy concerns) https://seraphicsecurity.com/learn/ai-browser/ai-browser-extensions-pros-cons-and-8- extensions-to-know-in-2026/ Seraphic Security",
        "\"Dia Browser Risks and Vulnerabilities\" (7-day AI memory as new attack surface) https://layerxsecurity.com/generative-ai/dia-browser-risks-and-vulnerabilities/ LayerX"
      ]
    },
    "priority": "high",
    "description": "Users install AI browser extensions and OS/IDE copilots that promise productivity boosts. These tools request broad permissions (\"read and change all your data on websites you visit,\" \"read files in this folder,\" etc.) and stream page…"
  },
  {
    "id": "dsgai17",
    "code": "DSGAI17",
    ...
    "detailedSections": {
      ...
      "mitigationTiers": {
        ...
        "tier3": [
          ...
          "RTO/RPO for AI pipeline components: Formally defined and contractually enforced recovery objectives for vector stores, model registries, and embedding pipelines as distinct infrastructure tiers. Scope: Buy and Build."
        ]
      },
      "knownCVEs": [
    ...
    "priority": "medium",
    "description": "RAG-dependent applications have a data integrity dependency that does not exist in traditional software: the correctness of every response is contingent on the freshness, consistency, and availability of the underlying vector store and…"
  },
  {
    "id": "dsgai18",
    "code": "DSGAI18",
    ...
    "detailedSections": {
      ...
      "knownCVEs": [
        "No canonical CVE.",
        "Anthropic - Detecting and Preventing Distillation Attacks - Large-scale query campaigns (e.g., DeepSeek) used to extract model capabilities for distillation. https://www.anthropic.com/news/detecting-and-preventing-distillation-attacks"
      ]
    },
    "priority": "high",
    "description": "Attackers iterate queries to infer membership (was X in training?) or reconstruct attributes/samples (model inversion). Embedding Inversion is a related technique where adversaries approximate original text via nearest neighbors or exploit…"
  },
  {
    "id": "dsgai19",
    "code": "DSGAI19",
    ...
    "detailedSections": {
      ...
      "attackVectors": [
        "Attackers may exploit labeling workflows by gaining access to annotation platforms, compromising reviewer endpoints, or infiltrating vendor environments. Insider threats are particularly relevant: labelers exposed to full records may intentionally or accidentally exfiltrate sensitive content. Compromised reviewer accounts can access datasets beyond what production users ever see. The attack vector lies in human visibility rather than model behavior."
      ],
      "examples": [
      ...
      "references": [
        "Ray Chowdhury et al.",
        "\"Differentially Private Reward Estimation with Preference Feedback\" (label DP for RLHF) https://arxiv.org/abs/2310.19733 arXiv",
        "Wu et al.",
        "\"Offline and Online KL-Regularized RLHF under Differential Privacy\" (label differential privacy in RLHF) https://openreview.net/pdf?id=oFRYuSvlAe openreview.net (Both papers explicitly discuss the privacy of human feedback/labels used to align generative models.)"
      ]
    },
    "priority": "medium",
    "description": "For RLHF, safety fine-tuning, and data quality review, human labelers are given raw prompts, completions, internal documents, or conversation transcripts. Tasks are outsourced to vendors or crowd platforms with weak controls. Labelers can…"
  },
  {
    "id": "dsgai20",
    "code": "DSGAI20",
    ...
    "priority": "high",
    "description": "Model Exfiltration Attacks (MEA), often referred to as distillation attacks, occur when an adversary uses legitimate API access to systematically and repeatedly probe a proprietary model (the \"teacher\" model) to extract its underlying…"
  },
  {
    "id": "dsgai21",
    "code": "DSGAI21",
    ...
    "detailedSections": {
      ...
      "references": [
        ...
        "For dataset integrity and provenance controls: see DSGAI13 (Synthetic Data, Anonymization & Transformation Pitfalls) and the Dataset Bill of Materials framework. Acknowledgements Authors The Scott Clinton, Board Co-chair, OWASP GenAI Co-founder - Strategy, Operations, Marketing Kyriakos \"Rock\" Lambros, Director of AI Standards and Governance, Zenity Emmanuel Guilherme Junior, OWASP GenAI Data Security Initiative Lead Contributors Alessandro Pignati, Lead AI Security Researcher at Neuraltrust Anitha Dakamarri, Lead Security Engineer at Donnelley Financial Solutions Bakul Singhal, Information Security Architect at Steve Madden Barbara Prevel, Digital Transformation Consultant Dan Sorensen, Founder & vCISO at Nexus Security Advisors Felipe Campos Penha PhD, Senior AI Engineer at Cargill Harish Ramachandran, Sr Director Program Management - Security and Compliance at SAP Hudson Pereira, CyberSecurity Consultant at Telef",
        "nica Tech Hussam Bteibet, Security Engineer at Tanium Illia Oleksiuk, Founder at Vorota AI Ivyonne Harris, Product Researcher, AI Platform Security at ServiceNow Kumaram Bujanand Logan Barr",
        ", Cybersecurity AI Analyst at Soci",
        "rale Oz Wasserman, Co-Founder & CPO at Opsin Praveen Dandin, Principal Software Engineer at Palo Alto Networks Rico Komenda, Application & AI Security Specialist at Adesso SE Roger Sanz, AI Governance and Security Lead at Plain Concepts Victor Lu, Independent Consultant Reviewers Matthew Houseman, 717 DEV Narendra Kumar Nutalapati, Independent Researcher",
        "AI Runtime Integrity Engineer Jonas von Glahn, Information Security & Compliance Lead at Blockbrain Joshua Nauman, Cybersecurity Test & Evaluation Engineer Rakesh Sharma, CYAIFI OWASP GenAI Security Project Sponsors We appreciate our Project Sponsors, funding contributions to help support the objectives of the project and help to cover operational and outreach costs augmenting the resources provided by the OWASP.org foundation. The OWASP GenAI Security Project continues to maintain a vendor neutral and unbiased approach. Sponsors do not receive special governance considerations as part of their support. Sponsors do receive recognition for their contributions in our materials and web properties. All materials the project generates are community developed, driven and released under open source and creative commons licenses. For more information on becoming a sponsor, visit the Sponsorship Section on our Website to learn more about helping to sustain the project through sponsorship. Project Sponsors: Sponsors list, as of publication date. Find the full sponsor list here. Page 102 genai.owasp.org Project Supporters Project supporters lend their resources and expertise to support the goals of the project. Accenture Cobalt Kainos PromptArmor AddValueMachine Inc Cohere KLAVAN Pynt Aeye Security Lab Inc. Comcast Klavan Security Group Quiq AI informatics GmbH Complex Technologies KPMG Germany FS Red Hat AI Village Credal.ai Kudelski Security RHITE aigos Databook Lakera SAFE Security Aon DistributedApps.ai Lasso Security Salesforce Aqua Security DreadNode Layerup SAP Astra Security DSI Legato Securiti AVID EPAM Linkfire See-Docs & Thenavigo AWARE7 GmbH Exabeam LLM Guard ServiceTitan AWS EY Italy LOGIC PLUS SHI BBVA F5 MaibornWolff Smiling Prophet Bearer FedEx Mend.io Snyk BeDisruptive Forescout Microsoft Sourcetoad Bit79 GE HealthCare Modus Create Sprinklr Blue Yonder Giskard Nexus stackArmor BroadBand Security, Inc. GitHub Nightfall AI Tietoevry BuddoBot Google Nordic Venture Family Trellix Bugcrowd GuidePoint Security Normalyze Trustwave SpiderLabs Cadea HackerOne NuBinary U Washington Check Point HADESS Palo Alto Networks University of Illinois Cisco IBM Palosade VE3 Cloud Security Podcast iFood Praetorian WhyLabs Cloudflare IriusRisk Preamble Yahoo Cloudsec.ai IronCore Labs Precize Zenity Coalfire IT University Copenhagen Prompt Security Supporters list, as of publication date. Find the full supporter list here."
      ]
    },
    "priority": "critical",
    "description": "Disinformation becomes a data security attack when an adversary deliberately introduces false, misleading, or manipulated data into a source that an AI system trusts -- a training corpus, a vector knowledge store, a retrieval index, a tool…"
  }
];
